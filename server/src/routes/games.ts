...
interface DBPlayer {
  ...
  open: DBCard[];
  hidden: DBCard[];
  revealedHidden: DBCard | null; // immer vorhanden, ggf. null
  finished?: boolean;            // <-- NEU: Spieler hat Runde beendet
}
...
function applyEndChecks(g: DBGame) {
  // markiere neu fertig gewordene Spieler
  for (const p of g.players) {
...
async function buildStateFor(g: DBGame, meId: string) {
  ...
  return {
    ...
    you: {
      ...
      open: me.open,
      hidden: me.hidden,
      revealedHidden: me.revealedHidden, // DBCard | null
...
/** ==== POST /games/:id/reveal-hidden  { index } ==== */
...
r.post('/:id/play', requireAuth, async (req: AuthRequest, res) => {
  ...
  if (source === 'hand') {
    ...
    const c = me.revealedHidden; // DBCard | null
